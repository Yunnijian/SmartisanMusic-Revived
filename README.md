...
本项目基于 [**Mangi-11**](https://github.com/Mangi-11/SmartisanMusic-Revived/commits?author=Mangi-11) 的 [**SmartisanMusic-Revived**](https://github.com/Mangi-11/SmartisanMusic-Revived) 项目二次开发。

[**Mangi-11**](https://github.com/Mangi-11/SmartisanMusic-Revived/commits?author=Mangi-11) 在早期版本加入过网易云音乐在线功能，我因此完全抛弃了网易云官方 App，但该功能在后续更新中被删除。本项目将网易云在线功能重新接入，并针对个人使用习惯做了一些改进和功能增加。与原项目的主要区别如下：

### 一、网易云在线功能

大部分代码来自 Mangi-11 已删除的旧仓库，并使用现代 Compose 重写了部分 UI。

1. **一起听**：可新建房间，或加入好友分享的房间，与好友一起听歌，支持歌单同步与播放进度同步。
2. **在线音乐分享**：支持分享在线歌曲。
3. **当前音质码率展示**：显示在播放页歌曲名下方，与作者并列展示。
4. **风格推荐**：每日推荐新增风格推荐。
5. **在线歌单导入**：在线歌单支持加入本地播放列表。
6. **账号「我喜欢」合并展示**：登录账号后，「我喜欢的音乐」中会显示账号喜欢歌单内的歌曲，与本地喜欢歌曲并列展示。
7. **登录交互优化**：目前支持手机号、二维码与 Web 三种登录途径。
8. **锤子风格下拉刷新**：云音乐页面新增锤子风格下拉刷新，组件动画来自 [**People-11**](https://github.com/People-11/SmartisanOS_APP_Port/commits?author=People-11) 的 [**SmartisanOS_APP_Port**](https://github.com/People-11/SmartisanOS_APP_Port) 项目内的 [Mail_7.1.0.apk](https://github.com/People-11/SmartisanOS_APP_Port/blob/master/Mail_7.1.0.apk)。

### 二、外观

1. 新增网易云音乐经典黑胶唱机样式可选，默认为原版样式。
2. 歌词页歌词不再直接截断，改为羽化淡出。

### 三、播放器

我的设备更新澎湃 OS 4 后，系统内置的音频解码器全部失效，导致未内置音频解码器的播放器无法正常播放 FLAC 等高品质音乐，因此新增内置 `media3-flac` 拓展模块用于解码。

### 四、歌词与 SuperLyric

引入 SuperLyric API，将当前播放的歌词（含逐字与翻译）发布给 SuperLyric，桌面歌词、悬浮歌词等模块可据此获取并显示歌词。未安装对应模块时静默降级，不影响播放。
...
Smartisan OS 已经退出历史舞台，本项目因此以锤子音乐 8.1.0 为视觉与交互基准，使用现代 Android 技术栈重新实现这款音乐播放器。界面全部采用 Jetpack Compose，通过自定义 Smartisan 组件延续原有的拟物质感、布局比例与动画节奏；媒体扫描、后台播放、队列、收藏、播放列表和数据持久化则全部基于公开 Android API 重写。在完整保留本地播放能力（读取并播放设备音频）的基础上，同时支持可选的网易云在线音乐：搜索、推荐、歌单、专辑与歌词浏览无需登录，账号歌单与「我喜欢」同步等账号能力需主动登录，登录是纯自愿的，凭据仅加密存储在设备本地。
...
- **可切换唱机样式**：内置「原版唱机」与「网易云音乐唱机」两套样式，设置中即时切换并持久化。原版沿用锤子音乐 8.1.0 的拟物唱盘；网易云样式取自网易云音乐 Mac 客户端 3.1.11 的官方唱片位图与唱针 SVG，唱片、唱臂、拖针映射与落针动画独立标定。
...
- 黑胶唱盘、唱针拖拽、搓碟与爆豆音
- 「原版唱机」与「网易云音乐唱机」两套唱机样式，设置-外观中即时切换并持久化
- 音频文件内嵌的静态、逐行及逐字歌词
...
云音乐（可选，浏览与搜索无需登录；账号相关能力需登录网易云账号）：
...
- 一起听：创建或接受邀请，双端同步播放与队列，展示连接时长
- 向 SuperLyric 提供实时歌词，支持桌面歌词等模块显示

## 权限与隐私

应用默认是本地播放器，`INTERNET` 权限用于在线音乐请求。访客无需登录即可浏览推荐、搜索与详情，未登录时播放走访客兜底；账号歌单、喜欢同步等账号能力需**主动登录网易云账号**。登录凭据经 `EncryptedSharedPreferences` 加密后仅保存在设备本地，不会明文落盘，也不会上传设备上的歌曲、封面、歌词或资料库信息。
...
| 播放 | Media3 `1.10.1`、ExoPlayer、MediaLibraryService、MediaSession、自编 libFLAC 扩展解码模块（`:media3-flac`） |
| 状态 | Lifecycle、StateFlow、Coroutines |
| 存储 | Room `2.8.4`、DataStore `1.2.1`、MediaStore |
| 在线 | 自实现网易云客户端（`HttpURLConnection` + AES/RSA 签名）、Coil 3（封面图加载） |
...
网易云唱机样式使用网易云音乐 Mac 客户端 3.1.11 的官方唱片位图 `outline.png` 与 `disc.png`。来源路径、版本与 SHA-256 见 [第三方声明](THIRD_PARTY_NOTICES.md)。
